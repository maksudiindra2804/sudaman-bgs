...
class TblAbsensiController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','export','import','editable','toggle','createmaster','lookupnip','update'),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['TblAbsensi']))
		{
			...
			try{
				...
				$model->attributes=$_POST['TblAbsensi'];
				$checkAwal=strtotime(date('Y-m-d H:i:s'));
				$checkDate=strtotime(date('Y-m-d 12:10:00'));
				$checkPertama=strtotime(date('Y-m-d 09:30:00'));
				$model->status='Lembur';
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['TblAbsensi']))
		{
			...
			try{
				$model->attributes=$_POST['TblAbsensi'];
				$model->absen_pulang=date('Y-m-d H:i:s');
				//$diff = date('Y-m-d H:i:s',strtotime($model->absen_pulang)) - date('Y-m-d H:i:s',strtotime($model->id_absen));
				//$model->total_jam=$interval;
				//echo $interval->format('%d days %h hours %i minutes');

				//$query = "SELECT absen_datang, absen_pulang, TIMESTAMPDIFF(HOUR, absen_datang, absen_pulang) AS total_jam FROM tbl_absensi";
				//$command =Yii::app()->db->createCommand($query);
				//$command->save();

				//$model->total_jam=$command;
				//$checkDate2=TblAbsensi::model()->findAllBySql("SELECT absen_datang FROM tbl_absensi WHERE id_absen='$id'");
				//$checkDate2 = TblAbsensi::model()->findAllBySql('SELECT absen_datang FROM tbl_absensi WHERE id_absen=:absen', array(':absen'=>$model->id_absen));
				...
				//$diff = (strtotime($model->absen_datang) - strtotime($model->absen_pulang) / (60 * 60 * 24));
				$messageType = 'info';
	...
	public function actionUpdateuser($id)
	{
		...
		if(isset($_POST['TblAbsensi']))
		{
			...
			try{
				$model->attributes=$_POST['TblAbsensi'];
				//$model->absen_pulang=date('Y-m-d H:i:s',$checkDatePulang);
				//$checkDatePulang = date('Y-m-d H:i:s');
				$checkDateAwal =isset($_GET['absen_datang']);

				$checkDate2 = strtotime($checkDateAwal=date('Y-m-d H:i'));
				$checkDate1 = strtotime($model->absen_pulang=date('Y-m-d H:i:s'));
				...
				if($model->save()){
					$transaction->commit();
					Yii::app()->user->setFlash($messageType, $message);
					$this->redirect(array('viewabsuser','id'=>$model->id_absen));
				}
			}
			catch (Exception $e){
				$transaction->rollBack();
	...
	*/
			...
			if (!empty($_FILES)) {
				...
				if (in_array(@$fileParts['extension'],$fileTypes)) {
					...
					while(!empty($sheetData[$baseRow]['A'])){
						...
						$absen_datang=  $sheetData[$baseRow]['C'];
						$status=  $sheetData[$baseRow]['D'];
						$keterangan=  $sheetData[$baseRow]['E'];
						$absen_pulang=  $sheetData[$baseRow]['F'];
						$total_jam=  $sheetData[$baseRow]['G'];

						$model2=new TblAbsensi;
						//$model2->id_absen=  $id_absen;
						$model2->nip=  $nip;
						$model2->absen_datang=  $absen_datang;
						$model2->status=  $status;
						$model2->keterangan=  $keterangan;
						$model2->absen_pulang=  $absen_pulang;
						$model2->total_jam=  $total_jam;
	...
	public function actionLookupnip()
	{
